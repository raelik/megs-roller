...
namespace :megs do
  ...
  desc 'Lists the characters in the database.'
  ...
  desc 'Deletes a user, all of their characters, and their roll logs.'
  task :delete_user, [:id] => :db_connect do |_t, args|
    ...
    if agree("Are you sure you want to delete #{user.username}, all of their characters, and their roll logs [y/N]? ") { |q| q.default = 'n'; q.default_hint_show = false }
      character_ids = MEGS::DB[:characters].where(user_id: user.id).to_a.map(&:id)
      MEGS::DB[:rolls].where(character_id: character_ids).changeset(:delete).commit
      MEGS::DB[:rolls].where(user_id: user.id).changeset(:delete).commit
  ...
  task :update_character, [:id] => :db_connect do |_t, args|
    ...
    character = MEGS::DB[:characters].by_pk(args.id.to_i).combine(:user).one
    ...
    if agree("Change owner of character (currently #{user.username}) [y/N]? ") { |q| q.default = 'n'; q.default_hint_show = false }
      ...
      while taken
        ...
        taken = MEGS::DB[:characters].where(user_id: changes[:user_id].to_i, name: changes[:name] || character.name).one
  ...
  task :delete_character, [:id] => :db_connect do |_t, args|
    ...
    if agree("Are you sure you want to delete #{character.name} for user #{user.username}, and their roll logs [y/N]? ") { |q| q.default = 'n'; q.default_hint_show = false }
      MEGS::DB[:rolls].where(character_id: character.id).changeset(:delete).commit
