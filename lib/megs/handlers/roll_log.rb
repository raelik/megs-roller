      ...
      def get
        ...
        if session
          log = (MEGS::DB[:rolls].combine(:user, :character).order { timestamp.desc }.limit(50).to_a.map do |r|
            ...
            roll_fields = r.filter { |k| %i(av ov ov_cs target total success cs ev rv rv_cs raps rolls).include?(k) }
            { timestamp: r[:timestamp].strftime('%Y-%m-%d %H:%M:%S.%7N %z'), user: r[:user][:name],
              character: r.dig(:character, :name), owner: owner }.merge(roll_fields)
          end)
        end
